...
| `UnityKnownWarningFilterPatch` | `BepInEx.Logging.UnityLogSource.OnUnityLogMessageReceived` | Known Unity warning spam for missing audio spatializer plugin setup, BoxCollider negative scale/size, disabled AudioSource playback, SteamValve empty AudioSource, and duplicate Static Lighting Sky warnings | Enabled by default as a log-only filter; filters only exact Unity warning message prefixes and only for `LogType.Warning`; Netcode lifecycle warnings are not filtered. | Low, global log callback patch. |
| `BepInExKnownLogNoiseFilterPatch` | `BepInEx.Logging.Logger.InternalLogEvent` | Known low-value BepInEx log noise from `RuntimeIcons`, `PathfindingLib`, and `LethalPerformance` | Enabled by default as a log-only filter; filters only exact source/level/message-shape matches and never filters Error or exception logs. | Low, global log callback patch that runs only when logs are emitted. |
...
| Patch | Target | Fixes | Trigger limit | Risk |
| --- | --- | --- | --- | --- |
| `PlayerControllerBNearOtherPlayersPatch` | `PlayerControllerB.NearOtherPlayers` | Missing player list/player transform dependencies | Disabled by default; only `Enabled` installs this hot-path replacement. `Auto` is treated as disabled. | Low. |
| `DocileLocustBeesAIUpdatePatch` / `CrawlerAIUpdatePatch` / `BushWolfEnemy*Patch` / `RedLocustBees*Patch` | Enemy `Update`, `LateUpdate`, and related hive AI checks | Missing enemy update dependencies | Disabled by default; only `Enabled` installs these hot-path patches. When enabled, skips only frames where required references are missing. `Auto` is treated as disabled. | Low. |
| `TerminalAccessibleObjectUpdatePatch` | `TerminalAccessibleObject.Update` | Missing terminal/radar dependencies | Disabled by default; only `Enabled` installs this hot-path patch. When enabled, skips frames until dependencies are ready and tries normal initialization when possible. `Auto` is treated as disabled. | Low. |
| `UnlockableSuitUpdatePatch` | `UnlockableSuit.Update` | Suit ID out of range | Disabled by default; only `Enabled` installs this hot-path patch. When enabled, lets vanilla run when `syncedSuitID` and dependencies are valid, and skips only invalid IDs or missing renderer/sync state. `Auto` is treated as disabled. | Low. |
...
| `EntranceTeleport` patches | Entrance teleport update/path lookup | Missing entrance/main entrance references | `EntranceTeleport.Update` is disabled by default; only `Enabled` installs this hot-path replacement and `Auto` is treated as disabled. The main entrance position fallback remains separately config-gated. | Low. |
